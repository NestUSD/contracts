...
pub fn start_liquidation(ctx: Context<StartLiquidation>) -> Result<()> {
    ...
    let collateral_value = collateral_value_for_raw_from_pyth(
    ...
    // The two-step path seizes the full vault first, then waits for real
    // USDC sale proceeds before restoring PSM backing and routing surplus.

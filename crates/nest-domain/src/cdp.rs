...
pub fn can_borrow(
    ...
    if checked_add(protocol.total_debt, borrow_amount)? > params.protocol_debt_cap {
        return Err(NestError::DebtCapExceeded);
    }
    if checked_add(params.collateral_debt_outstanding, borrow_amount)? > params.collateral_debt_cap
    {
        return Err(NestError::DebtCapExceeded);
    }
    Ok(())
